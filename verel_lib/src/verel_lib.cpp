...
int Init(const int argc, const char** argv)
{
  ...
  for (int i = 0; i < cfg::num_env; ++i)
    Simulator::env_list[i] = new VerelEnv(cfg::max_n);
  test_env = new VerelEnv(cfg::max_n);
...
double Test(const int gid)
{
  ...
  while (!test_env->isTerminal())
    {
      //      cost ++;
      if (test_env->graph->num_nodes - (*states[0]).size() == 1) {
	...
	break;
      ...
      std::cout << new_action << " ";
      sum_extra += new_action;
      test_env->step(new_action);
    }
  cost = std::accumulate(test_env->sum_rewards.begin(), test_env->sum_rewards.end(), 0);
  return cost;
}
